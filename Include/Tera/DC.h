...
namespace S1Data
{
  ...
  struct DCVector {
    friend FStream& operator<<(FStream& s, DCVector<T, TRawSerialization, TOffset, TSize>& vec)
    {
      ...
      {
        s.SerializeBytes((void*)vec.Data(), (FILE_OFFSET)vec.Size() * sizeof(T));
      }
      else
      {
  ...
  struct DCArray : DCVector<T, RawSerialization> {
    friend FStream& operator<<(FStream& s, DCArray<T, RawSerialization>& a)
    {
      ...
      if (RawSerialization)
      {
        s.SerializeBytes((void*)a.Container.data(), (FILE_OFFSET)a.Container.size() * sizeof(T));
      }
      else
      {
  ...
  protected:
    ...
    {
      s.SetPosition(s.GetPosition() + (FILE_OFFSET)size);
    }
  ...
  protected:
    ...
    class DCExporterImpl* Impl = nullptr;
  };
