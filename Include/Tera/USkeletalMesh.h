...
#include <Tera/FMeshCommon.h>
#include <Tera/FObjectArray.h>
...
public:
  ...
  int32 GetNumFaces(int32 lodIndex = 0) const
  {
    ...
    lodIndex = std::min<int32>(lodIndex, (FILE_OFFSET)LodModels.size() - 1);
    return LodModels.at(lodIndex).GetNumFaces();
  }

  bool AcceptVisitor(struct MeshTravallerData* importData, uint32 lodIdx, FString& error);
  bool ValidateVisitor(struct MeshTravallerData* importData, uint32 lodIdx, FString& error, bool& askUser, int32 warningIndex);
