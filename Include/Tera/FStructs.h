...
public:
  ...
  FGuid(int32 a, int32 b, int32 c, int32 d)
    : A((uint32)a)
    , B((uint32)b)
    , C((uint32)c)
    , D((uint32)d)
...
struct FRotator
{
  ...
  {
    return {(int32)truncf(Euler.Y * (32768.f / 180.f)), (int32)truncf(Euler.Z * (32768.f / 180.f)), (int32)truncf(Euler.X * (32768.f / 180.f))};
  }
...
public:
  ...
  using PackedType = typename FloatInfo::PackedType;
  using FloatType = typename FloatInfo::FloatType;
...
struct FPackedPosition
{
  ...
  operator FVector() const
  {
    return {Vector.X / 1023.f, Vector.Y / 1023.f, Vector.Z / 511.f};
  }
...
public:
  ...
  void AddIndex(int32 elementIndex)
  {
    ...
    if (ElementSize == sizeof(uint16))
    {
      Get16BitBuffer()[ElementCount - 1] = (uint16)elementIndex;
    }
    else
    {
...
public:
  ...
  FRawIndexBuffer(const FRawIndexBuffer& a)
  {
    ...
    {
      AllocateBuffer(ElementCount, (uint8)ElementSize);
...
struct FQuatFixed32NoW {
  ...
  void ToQuat(FQuat& result) const
  {
    ...
    result.Y = Y;
    result.Z = Z;
    result.W = wSquared > 0.f ? sqrtf(wSquared) : 0.f;
  }
