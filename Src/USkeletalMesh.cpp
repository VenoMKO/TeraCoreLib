...
std::vector<FSoftSkinVertex> FStaticLODModel::GetVertices(bool gpuBuffer) const
{
  ...
  if (gpuBuffer)
  {
    for (uint32 idx = 0; idx < VertexBufferGPUSkin.ElementCount; ++idx)
    {
...
bool USkeletalMesh::AcceptVisitor(MeshTravallerData* importData, uint32 lodIdx, FString& error)
{
  ...
  for (int32 idx = 0; idx < matmap.size(); ++idx)
  {
    ...
    {
      matmap[idx] = (int32)(std::find(importData->ObjectMaterials.begin(), importData->ObjectMaterials.end(), (*it).second) - importData->ObjectMaterials.begin());
    }
    else
    {
      matmap[idx] = 0;
  ...
  for (int32 faceIndex = 0; faceIndex < importData->Faces.size(); faceIndex++)
  {
    ...
    if (!section)
    {
      ...
      section->ChunkIndex = (uint16)lod.Chunks.size() - 1;
    ...
    for (int32 vertexIndex = 0; vertexIndex < 3; vertexIndex++)
    {
...

      {
        ...
        for (uint32 i = 0; i < influenceCount; i++)
        {
          ...
          if (it == chunk->BoneMap.end())
          {
            ...
            vertex.InfluenceBones[i] = (uint8)chunk->BoneMap.size() - 1;
          }
          else
          {
            vertex.InfluenceBones[i] = (uint8)(it - chunk->BoneMap.begin());
          }
      ...
      FSkinVertexMeta vertexMeta = { vertex, (uint32)importData->Wedges[face.wedgeIndices[vertexIndex]].pointIndex };
    ...
    {
      for (uint32 triangleIndice : triangleIndices)
      {
        sectionIndexBuffer->AddIndex(triangleIndice);
      }
    }
  }
  ...
  for (int32 sectionIndex = 0; sectionIndex < lod.Sections.size(); sectionIndex++)
  {
    ...
    existingSection.ChunkIndex = (uint16)lod.Chunks.size() - 1;
  }
  ...
  for (int32 sectionIndex = 0; sectionIndex < lod.Sections.size(); sectionIndex++)
  {
    ...
    lod.NumVertices += (uint32)chunkVertices.size();
    ...
    chunk.NumRigidVertices = (int32)chunk.RigidVertices.size();
    chunk.NumSoftVertices = (int32)chunk.SoftVertices.size();
  ...
  for (int32 sectionIndex = 0; sectionIndex < lod.Sections.size(); sectionIndex++)
  {
    ...
    for (uint32 idx = 0; idx < sectionIndexBuffer.GetElementCount(); ++idx)
    {
  ...
  int32 requiredBoneCount = (int32)RefSkeleton.size();
