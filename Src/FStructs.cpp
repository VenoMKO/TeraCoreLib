...
FGuid FGuid::Generate()
{
  ...
  if (!FAILED(CoCreateGuid(&v)))
  {
    ...
    return {vInt32[0], vInt32[1], vInt32[2], vInt32[3]};
  }
  return {};
}
...
{
  Vector.X = std::clamp<int32>((int32)trunc(inVector.X * 1023.0f), -1023, 1023);
  Vector.Y = std::clamp<int32>((int32)trunc(inVector.Y * 1023.0f), -1023, 1023);
  Vector.Z = std::clamp<int32>((int32)trunc(inVector.Z * 511.0f), -511, 511);
}
...
{
  return {Roll * (180.f / 32768.f), Pitch * (180.f / 32768.f), Yaw * (180.f / 32768.f)};
}
...
FRotator FMatrix::Rotator() const
{
  ...
  FRotator	Rotator = FRotator(
    (int32)round(atan2(XAxis.Z, sqrtf((XAxis.X* XAxis.X) + (XAxis.Y* XAxis.Y))) * 32768.f / M_PI),
    (int32)round(atan2(XAxis.Y, XAxis.X) * 32768.f / M_PI),
    0
  );

  const FVector SYAxis = FRotationMatrix(Rotator).GetAxis(1);
  Rotator.Roll = (int32)round(atan2(ZAxis | SYAxis, YAxis | SYAxis) * 32768.f / M_PI);
  return Rotator;
}
...
FRotator FQuat::Rotator() const
{
  ...
  const float RAD_TO_DEG = 180.f / (float)M_PI;
  FVector tmp;
  if (singularityTest < -SINGULARITY_THRESHOLD)
  {
    tmp.Y = -90.f;
    tmp.Z = atan2f(yawY, yawX) * RAD_TO_DEG;
    tmp.X = (float)FRotator::NormalizeAxis((int32)(-tmp.Z - (2.f * atan2f(X, W) * RAD_TO_DEG)));
  }
...
FColor FLinearColor::ToFColor(bool sRGB) const
{
  ...
  result.A = (uint8)floorf(a * 255.999f);
  result.R = (uint8)floorf(r * 255.999f);
  result.G = (uint8)floorf(g * 255.999f);
  result.B = (uint8)floorf(b * 255.999f);
