...
bool PskUtils::ExportSkeletalMesh(USkeletalMesh* sourceMesh, MeshExportContext& ctx)
{
    ...
    for (int32 triangleIndex = 0; triangleIndex < numTriangles; ++triangleIndex)
    {
      ...
      for (int32 pointIdx = 0; pointIdx < 3; ++pointIdx)
      {
        ...
        if (!exists)
        {
          ...
          for (int32 infIdx = 0; infIdx < MAX_INFLUENCES; ++infIdx)
          {
            influence.Bones.InfluenceBones[infIdx] = (uint8)vertex.BoneMap->at(vertex.InfluenceBones[infIdx]);
            influence.Wieghts.InfluenceWeights[infIdx] = vertex.InfluenceWeights[infIdx];
          }
        }
  ...
  PskChunkHeader skelHeader("REFSKELT", sizeof(FNamedBoneBinary), (int32)bones.size());
  s << skelHeader;
  ...
  PskChunkHeader inflHeader("RAWWEIGHTS", sizeof(FVertInfluence), (int32)influences.size());
  s << inflHeader;
...
bool PskUtils::ExportAnimationSequence(USkeletalMesh* sourceMesh, UAnimSequence* sequence, MeshExportContext& ctx)
{
  ...
  PskChunkHeader bonesHeader("BONENAMES", sizeof(FNamedBoneBinary), (int32)refBones.size());
  s << bonesHeader;
  for (int32 idx = 0; idx < refBones.size(); ++idx)
  {
    const FMeshBone& refBone = refBones[idx];
  ...
  animInfo.TotalBones = (int32)refBones.size();
  animInfo.KeyQuotum = sequence->NumFrames * (int32)refBones.size();
  animInfo.AnimRate = (float)sequence->GetFrameRate() * ctx.TrackRateScale;
  animInfo.TrackTime = sequence->SequenceLength * (float)sequence->GetFrameRate();
...
bool PskUtils::ExportAnimationSet(USkeletalMesh* sourceMesh, UAnimSet* animSet, MeshExportContext& ctx)
{
  ...
  else
  {
    ...
    PskChunkHeader infoHeader("ANIMINFO", sizeof(AnimInfoBinary), (int32)sequences.size());
    s << infoHeader;
