...
void AlignReadStream(FStream& s, const int32 alignment)
{
...
int32 UAnimSequence::GetRawFramesCount()
{
  ...
  {
    NumRawFrames = std::max<int32>((int32)t.PosKeys.size(), NumRawFrames);
  }
  for (const FRotationTrack& t : RotTracks)
  {
    NumRawFrames = std::max<int32>((int32)t.RotKeys.size(), NumRawFrames);
  }
  return NumRawFrames;
}
...
{
  const int32 NumTracks = (int32)RawAnimationData.size();
  ...
  for (int32 TrackIndex = 0; TrackIndex < NumTracks; ++TrackIndex)
  {
    ...
    const int32 PrevNumPosKeys = (int32)RawTrack.PosKeys.size();
    const int32 PrevNumRotKeys = (int32)RawTrack.RotKeys.size();
...
void UAnimSequence::UncompressTracks()
{
  ...
  if (KeyEncodingFormat != AKF_PerTrackCompression)
  {
    ...
    const int32 tracksCount = (int32)CompressedTrackOffsets.size() / 4;
  ...
  {
    const int32 NumTracks = (int32)CompressedTrackOffsets.size() / 2;
  }
#endif
}
