...
static size_t OggRead(void* ptr, size_t size, size_t nmemb, void* datasource)
{
  ...
  return src->ReadOgg(ptr, (uint32)(size * nmemb));
}
...
size_t ALDevice::AddSoundSource(ALSoundObserver* observer, class USoundNodeWave* sound)
{
  ...
  alBufferData(src->BufferId, vi->channels == 1 ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16, src->PcmData, (ALsizei)data_len, (ALsizei)vi->rate);
...
size_t ALSoundSource::ReadOgg(void* ptr, uint32 size)
{
  ...
  DataOffset += (FILE_OFFSET)sizeToRead;
  return sizeToRead;
}
...
int32 ALSoundSource::SeekOgg(uint64 offset, int whence)
{
  ...
  case SEEK_END:
    DataOffset = (FILE_OFFSET)(Sound->GetResourceSize() - offset);
    break;
  }
  return DataOffset;
}
