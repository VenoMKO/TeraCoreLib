...
T* ResampleData(const T* input, int32 width, int32 height, int32 newWidth, int32 newHeight)
{
  ...
  for (int32 newY = 0; newY < newHeight; ++newY)
  {
    for (int32 newX = 0; newX < newWidth; ++newX)
    {
      ...
      int32 x1 = std::min<int32>((int32)floor(x + 1.), width - 1);
      int32 y0 = (int32)floor(y);
      int32 y1 = std::min<int32>((int32)floor(y + 1.), height - 1);
...
bool UTerrain::GetWeightMapChannel(int32 idx, void*& data, int32& width, int32& height)
{
  ...
  if (uint8* channel = (uint8*)malloc(len))
  {
    ...
    else
    {
      if (idx == INDEX_NONE)
      {
        idx = (int32)Layers.size() - 1;
      }
      idx = (int32)Layers.size() - idx;
      int32 weightMapIdx = std::min<int32>((int32)floorf(fabs(float(idx) - .1f) / 4.f), (int32)WeightMapTextures.size() - 1);
