...
FString GetTempDir(const char* appName)
{
  auto result = std::filesystem::temp_directory_path() / (appName ? appName : "RealEditor");
  std::error_code err;
...
FString GetTempFilePath(const char* appName)
{
  std::error_code err;
...
void LZO::Compress(const void* src, FILE_OFFSET srcSize, void* dst, FILE_OFFSET& dstSize, bool concurrent)
{
  ...
  for (int32 idx = 0; idx < totalBlocks; ++idx)
  {
    int32 pos = (int32)((ptr - start) * sizeof(*start));
  ...
  dstSize = (FILE_OFFSET)((ptr - start) * sizeof(*start));
  *totalCompressedSize = (uint32)((ptr - dataStartPtr) * sizeof(*start));
  delete[] compressionInfo;
}
