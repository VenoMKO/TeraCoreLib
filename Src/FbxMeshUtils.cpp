...
// Reduce animation curves tangents
...
inline void SetCurveKey(FbxAnimCurve* curve, const FbxTime& time, float value, bool last)
{
  ...
  curve->KeySet(key, time, value, interpolation, tangentMode, .01f, .01f, FbxAnimCurveDef::eWeightedAll, .01f, .01f);
  if (last)
  {
...
FbxUtils::FbxUtils()
{
  ...
  sceneInfo->Original_ApplicationName.Set(AppName.c_str());
  sceneInfo->Original_ApplicationVersion.Set(AppVersion.c_str());
  sceneInfo->LastSaved_ApplicationVendor.Set("Yupi");
  sceneInfo->LastSaved_ApplicationName.Set(AppName.c_str());
  sceneInfo->LastSaved_ApplicationVersion.Set(AppVersion.c_str());
...
bool FbxUtils::ExportAnimationSequence(USkeletalMesh* sourceMesh, UAnimSequence* sequence, MeshExportContext& ctx)
{
  ...
  if (ctx.ExportMesh)
  {
    ...
    mesh->InitControlPoints((int)verticies.size());
    ...
    for (int32 idx = 0; idx < materials.size(); ++idx)
    {
      ...
      std::string matName = mat ? mat->GetObjectName().String().UTF8() : ("Material_" + std::to_string(idx + 1));
...
bool FbxUtils::ExportAnimationSet(USkeletalMesh* sourceMesh, UAnimSet* animSet, MeshExportContext& ctx)
{
  ...
  {
    ctx.ProgressMaxFunc((int32)sequences.size());
  }
  ...
  for (FbxNode* meshNode : meshNodes)
  {
    ...
    if (FbxSkin* skin = (FbxSkin*)static_cast<FbxGeometry*>(mesh)->GetDeformer(0, FbxDeformer::eSkin))
    {
      for (int32 clusterIndex = 0; clusterIndex < skin->GetClusterCount(); ++clusterIndex)
      {
        ...
        for (int32 controlPointIndex = 0; controlPointIndex < cluster->GetControlPointIndicesCount(); ++controlPointIndex)
        {
          ...
          influence.VertIndex = static_cast<uint32>(controlPointsOffset + controlPointIndices[controlPointIndex]);
        }
      }
    }
    else
    {
      ...
      for (int32 controlPointIndex = 0; controlPointIndex < controlPointsCount; ++controlPointIndex)
      {
        ...
        influence.VertIndex = static_cast<uint32>(controlPointsOffset + controlPointIndex);
      }
    }
    ...
    for (int32 uniqueUVIndex = 0; uniqueUVIndex < uniqueUVCount; uniqueUVIndex++)
    {
    ...
    for (int triangleIndex = 0; triangleIndex < triangleCount; triangleIndex++)
    {
      ...
      for (int32 vertexIndex = 0; vertexIndex < 3; ++vertexIndex)
      {
        ...
        tmpWedges[unrealVertexIndex].VertexIndex = (int32)controlPointsOffset + (int32)mesh->GetPolygonVertex(triangleIndex, vertexIndex);
      }

      bool hasUVs = false;
      for (int32 layerIndex = 0; layerIndex < uniqueUVCount; ++layerIndex)
      {
      ...
      for (int32 vertexIndex = 0; vertexIndex < 3; ++vertexIndex)
      {
        ...
        triangle.WedgeIndex[vertexIndex] = static_cast<uint32>(wedgesOffset + w);
      }
    }
    ...
    {
      ctx.ImportData.Points.emplace_back((float)p.mData[0], (float)p.mData[1], (float)p.mData[2]);
    }
...
bool FbxUtils::ExportStaticMesh(UStaticMesh* sourceMesh, int32 lodIdx, MeshExportContext& ctx, void** outNode)
{
  ...
  for (uint32 idx = 0; idx < lod->IndexBuffer.GetElementCount(); ++idx)
  {
...
bool FbxUtils::ExportSequence(USkeletalMesh* sourceMesh, UAnimSequence* sequence, void* boneNodes, void* animLayer, MeshExportContext& ctx)
{
  ...
  for (int32 frame = 0; frame < numRawFrames; ++frame)
  {
    ...
    for (int32 boneIndex = 0; boneIndex < targetBones.Size(); ++boneIndex)
    {
      ...
      for (; cidx < 3; ++cidx)
      {
        SetCurveKey(curves[cidx], t, (float)pos[cidx], last);
      }
      for (; cidx < 6; ++cidx)
      {
        SetCurveKey(curves[cidx], t, (float)rot[cidx - 3], last);
      }
