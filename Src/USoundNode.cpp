...
static size_t OggRead(void* ptr, size_t size, size_t nmemb, void* datasource)
{
  ...
  return travaller->ReadOgg(ptr, (uint32)(size * nmemb));
}
...
int32 ExportSoundCueNodeRecursive(FString& cueString, USoundNode* node, int32& index, bool loop, float scale)
{
  ...
  else if (c == USoundNodeMixer::StaticClassName())
  {
    ...
    {
      mixer->InputVolume.emplace_back(1.f);
    }
    for (int32 cIdx = 0; cIdx < node->ChildNodes.size(); ++cIdx)
    {
  ...
  else if (c == USoundNodeRandom::StaticClassName())
  {
    ...
    {
      tnode->Weights.emplace_back(1.f);
    }
    cueString += "\tChildren=(";
...
int32 SoundTravaller::SeekOgg(uint64 offset, int whence)
{
  ...
  case SEEK_END:
    DataOffset = (FILE_OFFSET)(DataSize - offset);
    break;
  }
...
bool SoundTravaller::GetOggInfo(FSoundInfo& info)
{
  ...
  if (vorbis_info* vi = ov_info(&vf, -1))
  {
    ...
    info.Duration = (float)ov_time_total(&vf, -1);
